...
- **階梯式資金池開火 + 3天 CD 冷卻期 (Powder Allocation & CD Logic)**：
  ...
  - **CD 提前解鎖條款**：若 CD 期間當前 Dist60 較上次加碼時更低 2% 以上，CD 自動提前解鎖允許再次加碼。
...
## ⑨ Current Sprint
v2.5.0 升級資金池 3天 CD 冷卻期控管、打折天數撫平器與 EWT 開盤心理準備卡！

## ⑩ Current Version
v2.5.0 (資金池 CD 控管、打折天數撫平器與 EWT 開盤心理準備卡升級版)
...
- **目前停止位置**: v2.5.0 升級資金池 CD 冷卻期、打折天數撫平器與 EWT 開盤心理準備卡全數完工！
...
### 📅 2026-07-30 資金池 CD 控管、打折天數撫平器與 EWT 開盤心理準備卡發布 (v2.5.0)
- **打折視窗與天數撫平器 (Missed-out Relief)**：新增 `calculatePhaseDurationAndRelief()` 函式，動態計算恐慌/極度恐慌位階連續持續天數與歷史平均天數（T1 11天 / T2 19天），有效撫平投資人錯過買點的焦慮感。
- **階梯式資金池開火 + 3天 CD 冷卻期**：實作 `calculatePowderAndCdStatus()` 控管機制，恐慌（T2）動用 10% 資金池、極度恐慌（T1）動用 20% 資金池，並於加碼後鎖定 3 個交易日 CD 冷卻期。若期間 Dist60 再下殺 2% 以上則自動觸發提前解鎖條款。
- **EWT 開盤心理準備卡**：升級 5 階夜盤 EWT 漲跌對應之開盤心理準備提示（非點數預估），並整合至老巴盤前 07:30 AI 導航 Prompt 中。
- **部署發布**：全數完成 GAS CLI 部署與 GitHub `main` 分支推播發布。
