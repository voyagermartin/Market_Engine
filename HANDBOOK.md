...
### 📅 2026-08-06 全站位階主體邏輯統一與長線年線警示解耦發布 (v2.7.5)
- **市場位階判定重構 (`Market_Engine_GAS.js`)**：
  - 根治「季線偏離度 +0.31% 處於 T3 順風中性」與「今日市場位階顯示 T5 狂熱」之全站設計哲學矛盾 Bug。
  - 將市場位階判定全站統一為以 **季線偏離度 (Dist60)** 為主體判定（在季線 +0.31% 時位階回歸 **T3 順風/中性**，常態定期定額照常執行 `🚀 明天照常自動扣款`）。
  - 將 **年線偏離度 (Dist240)** 高位 (+31.70% > P90) 解耦轉為純戰術提醒，附加於行動指引與資產配置說明中，不再強制將常態位階硬踩入 T5 狂熱。
- **試算表公式同步更新 (`RAW_HISTORY` / `DASHBOARD`)**：
  - 同步將 `applyHistoryLogFormulas` 與 `buildDashboardSheet` B15 公式改為以 Dist60 為主導判斷，確保 Sheet 與 Web App 全面邏輯對齊。
- **部署發布**：全數更新 `Market_Engine_GAS.js` 與 `HANDBOOK.md` 版本號至 `v2.7.5`，完成 `clasp push`、`clasp deploy` 與 GitHub `main` 推播發布。
